...
Five principles that resolve prior confusion:
...
5. **A micro-app is an open protocol contract; the runtime is interchangeable.** What a micro-app *is*, externally, is its **wire contract** — ACDC schemas + exn routes + IPEX verbs + the EGF — which is pure KERI protocol, not any one implementation (the template data-model spec: *"the bilateral conversation lives in the protocol, not in any single artifact"*). So **any KERI-capable party can participate** as a counterparty using *their own* stack (Locksmith, a loaded Service-AID, or a third-party KERI app); the template is one role's *authoring* of its side, the runtime is merely *a* provider of it. **Openness is a design choice set by the authz gate** (§4.1): a `credential` gate admits anyone holding the required ACDC (open participation); an `allowlist`/`aid` gate keeps it closed. *Participation ≠ authority* — a counterparty invokes, presents, and verifies, but the role-AID's signed compute is the authority everyone trusts.
